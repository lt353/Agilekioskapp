...
export function Floor2ListView({ onNavigate, onBack, onHome, onWelcome, canGoBack }: Floor2ListViewProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [roomsByCategory, setRoomsByCategory] = useState<Record<string, RoomData[]>>({});
  
  // Load room data from Supabase
  useEffect(() => {
    const roomData = getRoomsByFloorAndCategory(2);
    setRoomsByCategory(roomData);
  }, []);

  // Define category order for Floor 2
  const categoryOrder = [
    'Administrative Offices',
    'Faculty Offices - Hawaiian Studies',
    'Faculty Offices - Business & Hospitality',
    'Faculty Offices - Social Science',
    'Program / Service Offices',
    'Vacant / Unassigned Rooms',
  ];
      ...
      <div className="flex-1 overflow-hidden px-4 py-3 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pb-4">
          ...
          <div className="text-center py-4">
            <p className="text-sm text-[#999999]">End of Floor 2 Directory</p>
          </div>
        </div>
      </div>
    </div>
